<script lang="ts">
	...
	const button = tv({
		...
		variants: {
			variant: {
				primary:
					'bg-primary-500 text-white hover:bg-primary-600 active:bg-primary-700 focus-visible:ring-primary-500 shadow-md hover:shadow-lg',
				secondary:
				...
				whatsapp:
					'bg-green-500 text-white hover:bg-green-600 active:bg-green-700 focus-visible:ring-green-500 shadow-md hover:shadow-lg'
			},
			size: {
				sm: 'h-8 px-3 text-sm',
	...
	const classes = $derived(button({ variant, size, fullWidth, class: className }));
</script>
